  ...
  <div v-else class="min-h-screen flex flex-col bg-gray-50 font-sans relative overflow-hidden">
    ...
    <header class="relative z-10 px-8 py-6 flex justify-between items-center bg-white border-b border-neutral-400">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-2xl font-black uppercase tracking-widest text-neutral-800 isuzu-font">
            User <span class="text-red-600">Management</span>
          </h1>
          <p class="text-[10px] text-gray-500 uppercase tracking-[0.3em] font-bold">Access Control Matrix</p>
        </div>
      </div>

      <div class="flex items-center gap-4">
         <span class="px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest bg-red-50 text-red-600 border border-red-100">
        ...
        <div class="bg-white/40 backdrop-blur-none rounded-3xl p-6 border border-neutral-300 flex flex-wrap items-center gap-4">
          <div class="relative flex-1 min-w-[300px]">
            ...
            <input
              ...
              class="w-full pl-11 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-[10px] font-black uppercase tracking-widest focus:ring-2 focus:ring-red-500/20 focus:border-red-500 outline-none transition-all"
            />
          </div>

          <div class="flex flex-wrap items-center gap-3">
            <select v-model="selectedRole" class="px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-[10px] font-black uppercase tracking-widest focus:ring-2 focus:ring-red-500/20 focus:border-red-500 outline-none cursor-pointer">
              <option value="">ALL ROLES</option>
              <option v-for="role in roleOptions" :key="role" :value="role">{{ role.toUpperCase() }}</option>
            </select>

            <select v-model="selectedStatus" class="px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-[10px] font-black uppercase tracking-widest focus:ring-2 focus:ring-red-500/20 focus:border-red-500 outline-none cursor-pointer">
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden relative">
            ...
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="bg-gray-50/50 border-b border-gray-100">
                  <th class="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Administrator</th>
                  <th class="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Contact</th>
                  <th class="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Position & Role</th>
                  <th class="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Permission</th>
                  <th class="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Status</th>
                  <th class="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] text-center">Actions</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-50">
                <tr v-for="admin in paginatedAdmins" :key="admin.id" class="group hover:bg-red-50/30 transition-colors">
                  <td class="px-6 py-5">
                    <div class="flex items-center gap-3">
                      <div class="w-10 h-10 rounded-full bg-neutral-800 flex items-center justify-center text-white font-black text-xs uppercase">
                        {{ admin.firstName[0] }}{{ admin.lastName[0] }}
                      </div>
                      <div>
                        <p class="text-sm font-black text-neutral-800 uppercase tracking-tight">{{ admin.firstName }} {{ admin.lastName }}</p>
                        <p class="text-[10px] text-red-600 font-bold tracking-widest">@{{ admin.username }}</p>
                      </div>
                    </div>
                  </td>

                  <td class="px-6 py-5 text-xs font-bold text-neutral-600 italic">
                    {{ admin.email }}
                  </td>

                  <td class="px-6 py-5">
                    <div v-if="admin.isEditing" class="flex flex-col gap-1">
                      <select v-model="admin.selectedPosition" class="w-full bg-white border border-gray-200 rounded px-2 py-1 text-[10px] font-bold uppercase">
                        <option v-for="pos in positionOptions" :key="pos" :value="pos">{{ pos }}</option>
                      </select>
                      <select v-model="admin.selectedRole" class="w-full bg-white border border-gray-200 rounded px-2 py-1 text-[10px] font-bold uppercase">
                        <option v-for="role in roleOptions" :key="role" :value="role">{{ role }}</option>
                      </select>
                    </div>
                    <div v-else>
                      <p class="text-xs font-black text-neutral-800 uppercase">{{ admin.position || 'N/A' }}</p>
                      <p class="text-[10px] text-gray-400 uppercase font-bold">{{ admin.roleName || 'N/A' }}</p>
                    </div>
                  </td>

                  <td class="px-6 py-5">
                    <select v-if="admin.isEditing" v-model="admin.selectedPermission" class="w-full bg-white border border-gray-200 rounded px-2 py-1 text-[10px] font-bold uppercase">
                      <option v-for="perm in permissionOptions" :key="perm" :value="perm">{{ perm }}</option>
                    </select>
                    <span v-else class="px-3 py-1 bg-neutral-100 rounded-full text-[10px] font-black text-neutral-600 uppercase tracking-widest border border-neutral-200">
          ...
          <div class="px-8 py-5 bg-gray-50 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
              <select v-model="itemsPerPage" class="text-[10px] font-black bg-white border border-gray-200 rounded-lg px-2 py-1 outline-none">
                <option v-for="opt in itemsPerPageOptions" :key="opt" :value="opt">{{ opt }} PER PAGE</option>
              </select>
              <div class="flex items-center gap-1 ml-4">
                <button @click="currentPage = 1" :disabled="currentPage === 1" class="p-2 rounded-lg border border-gray-200 bg-white disabled:opacity-30 hover:text-red-600 transition-colors"><ChevronsLeft class="w-4 h-4" /></button>
                <button @click="currentPage--" :disabled="currentPage === 1" class="p-2 rounded-lg border border-gray-200 bg-white disabled:opacity-30 hover:text-red-600 transition-colors"><ChevronLeft class="w-4 h-4" /></button>
                <div class="flex items-center px-4 text-[10px] font-black text-neutral-800 italic uppercase">PAGE {{ currentPage }} / {{ totalPages }}</div>
                <button @click="currentPage++" :disabled="currentPage === totalPages" class="p-2 rounded-lg border border-gray-200 bg-white disabled:opacity-30 hover:text-red-600 transition-colors"><ChevronRight class="w-4 h-4" /></button>
                <button @click="currentPage = totalPages" :disabled="currentPage === totalPages" class="p-2 rounded-lg border border-gray-200 bg-white disabled:opacity-30 hover:text-red-600 transition-colors"><ChevronsRight class="w-4 h-4" /></button>
              </div>
            </div>
          </div>
        </div>
