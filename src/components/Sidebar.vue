<template>
  <div class="flex h-screen bg-neutral-100 font-sans overflow-hidden">
    ...
    <aside class="hidden md:flex md:flex-col w-64 bg-neutral-900 text-white border-r border-neutral-900 shadow-2xl flex-shrink-0">
      ...
      <nav class="flex-1 px-3 py-6 space-y-1 overflow-y-auto custom-scrollbar">
        ...
        <router-link v-if="accessiblePages.dashboard" to="/admin/dashboard" 
          ...
          <LayoutDashboard class="w-4 h-4 transition-transform duration-300 group-hover:scale-110 flex-shrink-0" /> 
        ...
        <router-link v-if="accessiblePages['user-management']" to="/admin/user-management" 
          class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-neutral-400 font-bold transition-all duration-200 hover:bg-white/5 hover:text-white group" 
          active-class="!bg-neutral-800 !text-white !shadow-lg !translate-x-1">
          <Users class="w-4 h-4 transition-transform duration-300 group-hover:scale-110 flex-shrink-0" /> 
        ...
        <router-link v-if="accessiblePages.approve" to="/admin/approve" 
          class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-neutral-400 font-bold transition-all duration-200 hover:bg-white/5 hover:text-white group" 
          active-class="!bg-neutral-800 !text-white !shadow-lg !translate-x-1">
          <CheckSquare class="w-4 h-4 transition-transform duration-300 group-hover:scale-110 flex-shrink-0" /> 
        ...
        <router-link v-if="accessiblePages.inventory" to="/admin/inventory" 
          class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-neutral-400 font-bold transition-all duration-200 hover:bg-white/5 hover:text-white group" 
          active-class="!bg-neutral-800 !text-white !shadow-lg !translate-x-1">
          <Package class="w-4 h-4 transition-transform duration-300 group-hover:scale-110 flex-shrink-0" /> 
        ...
        <router-link v-if="accessiblePages['transaction-in']" to="/admin/transaction-in" 
          class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-neutral-400 font-bold transition-all duration-200 hover:bg-white/5 hover:text-white group" 
          active-class="!bg-neutral-800 !text-white !shadow-lg !translate-x-1">
          <ArrowDownLeft class="w-4 h-4 text-emerald-500 transition-transform duration-300 group-hover:scale-110 flex-shrink-0" /> 
          <span class="flex-1 text-left text-[10px] uppercase tracking-wider">Transaction In</span>
        </router-link>

        <router-link v-if="accessiblePages['transaction-out']" to="/admin/transaction-out" 
          class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-neutral-400 font-bold transition-all duration-200 hover:bg-white/5 hover:text-white group" 
          active-class="!bg-neutral-800 !text-white !shadow-lg !translate-x-1">
          <ArrowUpRight class="w-4 h-4 text-red-500 transition-transform duration-300 group-hover:scale-110 flex-shrink-0" /> 
          <span class="flex-1 text-left text-[10px] uppercase tracking-wider">Transaction Out</span>
        </router-link>
        ...
        <router-link v-if="accessiblePages.settings" to="/admin/settings" 
          class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-neutral-400 font-bold transition-all duration-200 hover:bg-white/5 hover:text-white group" 
          active-class="!bg-neutral-800 !text-white !shadow-lg !translate-x-1">
          <Settings class="w-4 h-4 transition-transform duration-300 group-hover:scale-110 flex-shrink-0" /> 
        ...
        <router-link v-if="isMasterAdmin" to="/admin/page-control" 
          ...
          <ShieldAlert class="w-4 h-4 transition-transform duration-300 group-hover:scale-110 flex-shrink-0" /> 
    ...
    <div class="flex-1 flex flex-col min-w-0 bg-neutral-100 relative">
      ...
      <header class="md:hidden h-14 bg-neutral-900 border-b border-neutral-800 flex items-center justify-between px-4 flex-shrink-0 z-30">
      ...
      <nav class="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[92%] bg-neutral-900 backdrop-blur-lg h-13 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] z-50 flex items-center justify-around px-2 border border-white/10">
        ...
        <div class="relative -mt-10">
          ...
          <transition name="pop">
            <div v-if="isLogisticsOpen" class="absolute bottom-20 left-1/2 -translate-x-1/2 w-52 bg-neutral-900 rounded-3xl shadow-2xl border border-white/10 p-2 flex flex-col gap-1 z-50">
               <router-link v-if="accessiblePages.inventory" to="/admin/inventory" @click="isLogisticsOpen = false" class="flex items-center gap-3 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-neutral-300 hover:bg-white/5 rounded-2xl transition-colors">
                <Package class="w-4 h-4 text-neutral-500" /> Inventory
              </router-link>
              <router-link v-if="accessiblePages['transaction-in']" to="/admin/transaction-in" @click="isLogisticsOpen = false" class="flex items-center gap-3 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-neutral-300 hover:bg-white/5 rounded-2xl transition-colors">
                <ArrowDownLeft class="w-4 h-4 text-emerald-500" /> Stock In
              </router-link>
